...
I first encountered VirtualSmb through Windows Sandbox's MappedFolders, which use VSMB to share host directories into the sandbox VM. When I consolidated my Windows Sandbox and Hyper-V Linux VMs onto [NanaBox](https://github.com/M2Team/NanaBox) (an HCS-based VM manager), I wanted the same shared folder experience for full Windows guests. In late 2025, hcsshim [open-sourced](https://github.com/microsoft/hcsshim) the GCS sidecar's VSMB initialization code (`internal/gcs-sidecar/vsmb.go`), which made it possible to implement Windows guest support without reverse engineering — the full discussion is in [NanaBox#49](https://github.com/M2Team/NanaBox/issues/49). [NanaRun's VirtualSmb utility](https://github.com/M2Team/NanaRun/blob/main/VirtualSmb/VirtualSmb.cpp) further documents the `LanmanRedirector` initialization path (`FSCTL_LMR_START_INSTANCE` / `FSCTL_LMR_BIND_TO_TRANSPORT`) and its fallback chain for versioned request structures across Windows build numbers 14393 through 28000 before binding the redirector to the VMBus transport.
